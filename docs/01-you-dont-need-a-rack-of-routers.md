...
CML, which is why this workshop provides no image.
...
takes about thirty minutes to set up and will carry you through the series.
...
```sh
curl -LO https://github.com/joshgilby/quelaag/releases/download/milestone-1/quelaag-milestone-1.tar.gz
...
! lab/README.md) FIXME; the device stores each one as a type-8 hash at boot.
...
like a fleet — later articles rely on this.
...
# The lab's documented fake credentials — the authoritative list is in lab/README.md. FIXME
USERNAME = "admin"
...
There is the payoff, sitting on your screen: `$8$…` strings where
...
hashes have nothing in common. Yours, off the same baseline, will match neither. What that
...
The obvious answer is to restart the container. Let's confirm  `r1` is running and reachable before restarting it:
...
will. Inside that container live two processes: `iol.bin`, which is the device, and
...
— *the reboot broke it* — is wrong. The device is fine. It is also gone until you
...
survives even a real reboot. This is the section's third wrong conclusion, and the
...
Now connect to `r1`, save the configuration manually, watch for the `[OK]`, and reboot a third time:
...
[quelaag-milestone-1.tar.gz](https://github.com/joshgilby/quelaag/releases/tag/milestone-1). FIXME
